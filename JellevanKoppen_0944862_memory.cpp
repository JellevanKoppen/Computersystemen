...
void sort() {
  ...
  while (!sorted) {
    ...
    for (int i = 0; i < noOfBlocks - 1; i++) {
      if (addr[i] > addr[i + 1]) {
		tmp = addr[i];				// Tijdelijke variabele om gegevens in te bewaren
		addr[i] = addr[i+1];		// De twee adressen worden omgewisseld
		addr[i+1] = tmp;
		tmp = length[i];
		length[i] = length[i+1];
		length[i+1] = tmp;
...
void allocate(long size) {
  ...
  if (f == -1) {
	...
	// Alloceert het geheugen grootte en geeft het beginadres terug
	addr[noOfBlocks] = f;
	length[noOfBlocks] = size;
	noOfBlocks++;
    cout << "Startingaddress: " << f << endl;
  }
}
...
long freeMem() {
  long total = getMem();
  for (int i = 0; i < noOfBlocks; i++) {
	  // Geeft de totale hoeveelheid vrij geheugen terug
	  total -= length[i];
  }
  return total;
}
...
string retrieveChar(long a) {
  string ret;
  char character = (char) recallByte(a);
  ret += character
  return ret;
}
...
string retrieveFloat(long a) {
  ...
  number = sign * pow(2,exponent) * ((float)mantissa/8388608);
